...
| Path | Purpose |
| --- | --- |
| `index.html` | Deployed build. Single self-contained file — fonts, runtime and the 3D module are inlined, so it works fully offline. |
...
Open `src/StressPoint-Public.dc.html` to work on copy and layout. `index.html` is a compiled artifact — regenerate it rather than editing it by hand.
